...
const app = {
  ...
  renderItem(item) {
    ...
    titleText.textContent = `${item.title} - ${item.dueDate} - ${item.dueTime}`; // 할 일 제목과 마감 기한을 텍스트로 설정
  ...
  init() {
    ...
    this.form.addEventListener("submit", (event) => {
      ...
      const dueTime = this.dueTimeInput.value;

      if (title && dueDate) {
        // 할 일 제목과 마감 기한이 모두 입력되었는지 확인
        const item = todoList.add(title, dueDate, dueTime); // To-Do 항목 리스트에 항목 추가
        this.renderItem(item); // 추가된 항목 렌더링
        this.titleInput.value = ""; // 입력 필드 초기화
        this.dueDateInput.value = ""; // 입력 필드 초기화
        this.dueTimeInput.value = ""; // 입력 필드 초기화
      }
    });
  },
};
